      ...
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-12">
          <div class="inline-flex items-center pl-1 pr-4 py-1 rounded-full bg-orange-500/10 mb-4">
            <span class="text-sm font-semibold text-gray-800 mr-2 rounded-full py-2 px-4 bg-orange-400">Édition</span>
            <span class="text-sm text-gray-300">Modifier le travailleur</span>
          </div>
        ...
        <div v-else-if="!worker" class="bg-red-500/10 backdrop-blur-lg border border-red-500/20 rounded-2xl p-6 mb-8">
          <div class="flex items-start">
            <svg class="h-6 w-6 text-red-400 mr-3 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
        ...
        <div v-else-if="success" class="bg-green-500/10 backdrop-blur-lg border border-green-500/20 rounded-2xl p-6 mb-8">
          <div class="flex items-start">
            ...
            <div class="flex-1">
              <span class="text-green-300">Travailleur mis à jour avec succès !</span>
              <div class="mt-4 flex space-x-3">
                <NuxtLink :to="`/workers/${id}`" class="inline-flex items-center px-4 py-2 bg-orange-500/20 hover:bg-orange-500/30 text-orange-400 rounded-lg transition-all duration-200 border border-orange-500/20">
        ...
        <div v-else class="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 overflow-hidden">
          ...
          <div class="flex items-center p-8 border-b border-white/10">
            ...
            <div class="flex-1">
              ...
              <p class="text-gray-400 text-sm">{{ worker.first_name }} {{ worker.last_name }} - ID: #{{ worker.id }}</p>
            </div>
          </div>
              ...
              <div class="bg-white/5 rounded-lg p-6 border border-white/10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div class="space-y-2">
                      <div class="flex items-center">
                        <input type="checkbox" id="can_copy" value="can_copy" v-model="writingSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_copy" class="ml-2 text-sm text-gray-300">Peut copier</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_write_letters" value="can_write_letters" v-model="writingSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_write_letters" class="ml-2 text-sm text-gray-300">Peut écrire des lettres</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_write_words" value="can_write_words" v-model="writingSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_write_words" class="ml-2 text-sm text-gray-300">Peut écrire des mots</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_write_sentences" value="can_write_sentences" v-model="writingSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_write_sentences" class="ml-2 text-sm text-gray-300">Peut écrire des phrases</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_write_texts" value="can_write_texts" v-model="writingSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_write_texts" class="ml-2 text-sm text-gray-300">Peut écrire des textes</label>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  <div>
                    ...
                    <div class="space-y-2">
                      <div class="flex items-center">
                        <input type="checkbox" id="can_count" value="can_count" v-model="calculationSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_count" class="ml-2 text-sm text-gray-300">Peut compter</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_perform_operations" value="can_perform_operations" v-model="calculationSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_perform_operations" class="ml-2 text-sm text-gray-300">Peut effectuer des opérations</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_add" value="can_add" v-model="calculationSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_add" class="ml-2 text-sm text-gray-300">Peut additionner</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_subtract" value="can_subtract" v-model="calculationSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_subtract" class="ml-2 text-sm text-gray-300">Peut soustraire</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_multiply" value="can_multiply" v-model="calculationSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_multiply" class="ml-2 text-sm text-gray-300">Peut multiplier</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_divide" value="can_divide" v-model="calculationSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_divide" class="ml-2 text-sm text-gray-300">Peut diviser</label>
                      </div>
                      <div class="flex items-center">
                        <input type="checkbox" id="can_solve_problems" value="can_solve_problems" v-model="calculationSkillsArray" class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded">
                        <label for="can_solve_problems" class="ml-2 text-sm text-gray-300">Peut résoudre des problèmes</label>
                      </div>
                    </div>
                  </div>
...
const formData = ref({
  ...
  // Orientation spatio-temporelle (JSON)
...
// Helper pour initialiser les tableaux depuis les strings
const initializeSkillsArrays = () => {
  if (formData.value.writing_skills) {
    writingSkillsArray.value = formData.value.writing_skills.split(',').map(s => s.trim()).filter(s => s);
  }
  if (formData.value.calculation_skills) {
...
onMounted(async () => {
  try {
    ...
    if (!workerResponse.ok) {
      ...
      throw new Error(`Erreur HTTP: ${workerResponse.status}`);
    }
    
    const workerData = await workerResponse.json();
    worker.value = workerData.data || workerData;
    
    // Initialiser le formulaire avec les données du travailleur
    formData.value = {
      // Informations de base
      first_name: worker.value.first_name || '',
      last_name: worker.value.last_name || '',
      birth_date: worker.value.birth_date || '',
      internal_code: worker.value.internal_code || '',
      contact_info: worker.value.contact_info || '',
      
      // Informations ESAT
      esat_id: worker.value.esat_id || null,
      entry_date_esat: worker.value.entry_date_esat || '',
      work_regime: worker.value.work_regime || '',
      part_time_percentage: worker.value.part_time_percentage || null,
      ...
      // Secteurs d'activité et vigilance
      activity_sectors: worker.value.activity_sectors || '',
      activity_sectors_other: worker.value.activity_sectors_other || '',
      vigilance_points: worker.value.vigilance_points || '',
      vigilance_actions: worker.value.vigilance_actions || '',
      
      // Orientation spatio-temporelle (JSON)
      spatial_temporal_orientation: worker.value.spatial_temporal_orientation || null,
      
      // Compétences
      reading_skills: worker.value.reading_skills || '',
      writing_skills: worker.value.writing_skills || '',
      calculation_skills: worker.value.calculation_skills || '',
      computer_skills: worker.value.computer_skills || '',
      computer_skills_comments: worker.value.computer_skills_comments || '',
      
      // Évaluations (JSON)
      professional_evaluation: worker.value.professional_evaluation || null,
      professional_evaluation_comments: worker.value.professional_evaluation_comments || '',
      social_relations: worker.value.social_relations || null,
      social_relations_comments: worker.value.social_relations_comments || '',
      
      // Documents et capacités
      has_cv: worker.value.has_cv || false,
      has_motivation_letter: worker.value.has_motivation_letter || false,
      has_cpf_account: worker.value.has_cpf_account || false,
      ...
      professional_experiences: worker.value.professional_experiences || null,
      declined_applications: worker.value.declined_applications || null,
      exceptional_experiences: worker.value.exceptional_experiences || '',
      project_difficulties: worker.value.project_difficulties || '',
      professional_project_clarity: worker.value.professional_project_clarity || null,
      ordinary_work_capacity: worker.value.ordinary_work_capacity || null
    };
    
    // Initialiser les tableaux de compétences
    initializeSkillsArrays();
    ...
    try {
      ...
      if (esatsResponse.ok) {
        ...
        esats.value = esatsData.data || esatsData;
      }
    } catch (esatsErr) {
...
const submitForm = async () => {
  try {
    ...
    if (dataToSend.professional_project_clarity !== null && dataToSend.professional_project_clarity !== '') {
      dataToSend.professional_project_clarity = Number(dataToSend.professional_project_clarity);
    }
    if (dataToSend.ordinary_work_capacity !== null && dataToSend.ordinary_work_capacity !== '') {
      dataToSend.ordinary_work_capacity = Number(dataToSend.ordinary_work_capacity);
    }
