  ...
  <div class="w-full">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start mb-6">
      <div class="mb-4 md:mb-0">
        <h1 class="text-2xl font-semibold text-gray-800 mb-2">Liste des Travailleurs</h1>
        <p class="text-sm text-gray-500 flex items-center">
          <span>Dashboard</span>
          <svg class="h-3 w-3 mx-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
          </svg>
          <span class="text-green-600 font-medium">Travailleurs</span>
        </p>
      </div>
      <NuxtLink to="/workers/create" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200">
    ...
    <!-- État vide -->
    <div v-else-if="workers.length === 0" class="flex flex-col items-center justify-center py-12 bg-white rounded-lg shadow">
      <div class="w-20 h-20 text-gray-300 mb-4">
        <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
        </svg>
      </div>
      <h3 class="text-lg font-semibold text-gray-800 mb-2">Aucun travailleur trouvé</h3>
      <p class="text-gray-500 mb-6 max-w-sm text-center">Commencez par ajouter un travailleur au système</p>
      <NuxtLink to="/workers/create" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200">
        <svg class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
        </svg>
    ...
    <!-- Card principale avec table des travailleurs -->
    <div v-else class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center p-4 border-b border-gray-200">
        <div class="flex items-center mb-3 md:mb-0">
          ...
          <h2 class="text-lg font-semibold text-gray-800">Travailleurs enregistrés</h2>
        </div>
        <div class="w-full md:w-auto relative">
          <input
            type="text"
            placeholder="Rechercher un travailleur..."
            class="w-full md:w-64 pl-4 pr-10 py-2 border border-gray-300 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200"
          />
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="worker in workers" :key="worker.id" class="hover:bg-green-50 transition-colors duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="font-medium text-gray-900">{{ worker.first_name || '-' }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="font-medium text-gray-900">{{ worker.last_name || '-' }}</div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                <div class="flex items-center justify-center space-x-2">
                  <NuxtLink :to="`/workers/${worker.id}`" class="p-1.5 rounded-full hover:bg-blue-50 text-blue-500 hover:text-blue-600 transition-colors duration-200" title="Voir les détails">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                    </svg>
                  </NuxtLink>
                  <NuxtLink :to="`/workers/${worker.id}/edit`" class="p-1.5 rounded-full hover:bg-green-50 text-green-500 hover:text-green-600 transition-colors duration-200" title="Modifier">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                    </svg>
                  </NuxtLink>
                  <button @click="confirmDelete(worker)" class="p-1.5 rounded-full hover:bg-red-50 text-red-500 hover:text-red-600 transition-colors duration-200" title="Supprimer">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="px-6 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between bg-gray-50 border-t border-gray-200">
          <div class="text-sm text-gray-500 mb-2 sm:mb-0">
            Affichage de <span class="font-medium text-gray-700">1-{{ workers.length }}</span> sur <span class="font-medium text-gray-700">{{ workers.length }}</span> travailleurs
          </div>
      ...
      <div class="bg-white rounded-lg shadow-xl overflow-hidden max-w-md w-full animate-modal-appear">
        ...
        <div class="p-5">
          <p class="text-gray-700 mb-4">
            Êtes-vous sûr de vouloir supprimer le travailleur "<strong>{{ workerToDelete?.first_name }} {{ workerToDelete?.last_name }}</strong>" ?
          </p>
        ...
        <div class="px-5 py-4 bg-gray-50 flex justify-end space-x-3">
          <button @click="cancelDelete" class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200">
            Annuler
          </button>
          <button @click="deleteWorker" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200">
...
const deleteWorker = async () => {
  ...
  try {
    ...
    console.error(err);
    // Keep modal open on error to show message if needed, or handle error display differently
  }
};
</script>
