...
const user = ref(null); // To store fetched user data
...
// Fetch User data and ESATs
onMounted(async () => {
  try {
    // Fetch User
    const userResponse = await fetch(`/api/users/${id.value}`);
    if (!userResponse.ok) {
      if (userResponse.status === 404) throw new Error('Utilisateur non trouvé');
      throw new Error(`Erreur HTTP (utilisateur): ${userResponse.status}`);
    }
...
const resetForm = () => {
  if (originalData.value) {
    formData.value = { ...originalData.value, password: '' }; // Reset password field too
  }
  error.value = null;
  success.value = false; // Hide success message on reset
};
...
const submitForm = async () => {
  ...
  try {
    ...
    // Remove password if it's empty (don't update it)
    ...
    // Update original data for potential reset, keep password field empty
    originalData.value = { ...formData.value, password: '' };
    success.value = true;
    
  } catch (err) {
    error.value = `Erreur lors de la mise à jour de l'utilisateur: ${err.message}`;
    console.error(err);
  } finally {
