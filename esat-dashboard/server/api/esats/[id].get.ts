...
import { db, EsatTable } from '../../utils/db';
import { Selectable } from 'kysely';

// Define the type for the ESAT data we expect to select and return
type EsatSelectable = Selectable<EsatTable>;

export default defineEventHandler(async (event): Promise<{ status: string; data: EsatSelectable }> => {
  ...
  try {
    ...
    if (currentUser.role !== 'super_admin') {
      if (currentUser.role !== 'admin_esat' && currentUser.role !== 'user_esat') {
        throw createError({
          statusCode: 403,
          statusMessage: 'Insufficient permissions to access ESAT details'
        });
      }
    ...
    if (!esat) {
      throw createError({ statusCode: 404, statusMessage: 'ESAT not found' });
    }

    console.log(`Successfully fetched ESAT with ID ${esatId}.`);
    return {
      status: 'success',
      data: esat as EsatSelectable
    };
    ...
    throw createError({
      ...
      statusMessage: error.statusMessage || `Failed to fetch ESAT with ID ${esatId}`,
