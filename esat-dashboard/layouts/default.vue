  ...
  <div class="min-h-screen main-gradient-bg relative overflow-hidden">
    ...
    >
      ...
      <nav class="p-4 flex-1">
        <ul class="space-y-2">
          <!-- Navigation conditionnelle bas�e sur les permissions -->
...
// �tat de la sidebar mobile
const isSidebarOpen = ref(false);

// Fonctions pour contr�ler la sidebar
const toggleSidebar = () => {
...
const handleResize = () => {
  if (window.innerWidth >= 1024) {
    ...
    isSidebarOpen.value = false; // Ferm�e par d�faut sur mobile
  }
};
...
// Computed pour v�rifier si l'onglet "Mon ESAT" doit �tre actif
const isMyEsatActive = computed(() => {
  ...
  // Actif si on est sur /my-esat ou sur la page de d�tails de son propre ESAT
...
const handleLogout = async () => {
  try {
    ...
    // Forcer la redirection vers login m�me en cas d'erreur
...
onMounted(async () => {
  ...
  try {
    // G�rer la taille d'�cran initiale
    handleResize();
    
    // �couter les changements de taille d'�cran
    window.addEventListener('resize', handleResize);
    
    // R�cup�rer les composables d'authentification
